<script lang="ts">
  ...
  const TypedSchema2 = typedFieldPipe(TypedSchema, (d) => [
    d(['text1'], [
      d.outputChange((fn) => {
        fn([{ list: ['..', 'radio1'], output: 'onIndexChange' }]).subscribe(
          (change) => {
  ...
  const field = convertToField(() => TypedSchema2, undefined, () => ({
    ...
    {#snippet children(cvaa: any)}
      <input
        ...
        oninput={(e: Event) => {
          ...
          cvaa.valueChange(raw === '' ? undefined : Number(raw));
        }}
        onblur={cvaa.touchedChange}
      />
    {/snippet}
    ...
    {#snippet children(cvaa: any)}
      <label class="label cursor-pointer justify-start gap-2">
        <input
          ...
          disabled={cvaa.disabled}
          onchange={(e: Event) =>
            cvaa.valueChange((e.currentTarget as HTMLInputElement).checked)
          }
          onblur={cvaa.touchedChange}
        />
